...
(function($, window, undefined){
  ...
  $.fn.twitterTimeline = function(user, config){
    var el = this,
        url = '',
        linkify = function(t){
          return t.replace(/(https?:\/\/)([\w\-:;?&=+.%#\/]+)/gi, '<a href="$1$2">$2</a>')
    ...
    $.getJSON(url, function(res){
      ...
      el.empty();
      $.each(res, function(idx, tweet){
        if (i <= 0) return;
        if (config.show_replies || tweet.in_reply_to_user_id_str == null){
          c = '<li><p><a class="tweet-link" href="http://twitter.com/'+user+'/status/'+tweet.id_str+'">'+prettydate(tweet.created_at)+'</a>'+linkify(tweet.text).replace(/\n/g, '<br>')+'</p></li>';
          $(c).appendTo(el);
          i--;
        }
      });
